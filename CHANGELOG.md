...
- **2026-09-22 (#3) — issue-backed reviews use the repository tracker contract.** Issue #25 adds
  ...
  plus corrections verified against explicit owner direction or the current agreement before the
  Spec axis. It records A7's `gh` surface, B2's filing and orchestrator-only issue-operation
  authority, and the `Refs`-into-`develop` / `Closes`-into-`master` boundary. Review completed
  the contract by making PR-body references trigger the same fetch, treating comment claims as
  untrusted until verified, and banning every GitHub closing-keyword form on feature PRs.
